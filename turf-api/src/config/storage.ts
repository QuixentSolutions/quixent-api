...
// so Turf's blobs don't sit inside another product's container — same Azure
// account/connection string though. Mirrors thallu-vandi-api's storage config.
...
export async function uploadToAzure(key: string, buffer: Buffer, contentType: string): Promise<void> {
  ...
  // access: 'blob' — anonymous read of individual blobs (not container
  // listing), so the app's plain <Image> GETs resolve without a SAS token.
